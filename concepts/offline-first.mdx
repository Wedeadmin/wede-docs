...
| Channel | Description | Best For | Status |
| --- | --- | --- | --- |
| **REST Full** | Standard HTTPS — primary channel | Normal operations | Live |
| **REST Compressed** | Optimised low-bandwidth HTTPS | Degraded internet | Live |
| **Structured Protocols** | Compact structured transmission via SMS | When internet is unavailable | Live |
| **Voice** | Voice call delivery | Critical alerts, no data | In pilot rollout |
| **LoRa** | Long-range IoT radio — up to 15 km range | Remote areas, rural infrastructure | Roadmap |
| **Satellite** | Global satellite coverage via third-party integration | Extreme environments, conflict zones, no cellular | Roadmap |
| **Edge** | Local edge processing — zero external dependency | Air-gapped environments, ultra-low latency | Roadmap |

<Note>
  Channels marked *Roadmap* are architecture-ready (schema, pricing and provider integration layer in place) but do not yet carry production traffic. Voice is built and in pilot rollout with our voice provider. Structured Protocols (SMS) is the only fallback channel with a live delivery connector today.
</Note>
...
| **sms\_only** | Internet unavailable | Structured Protocols (SMS) live; Voice in pilot rollout |
| **offline** | All internet channels unavailable | Events queue locally with guaranteed sync on reconnection; LoRa, Satellite and Edge delivery are on our roadmap |
