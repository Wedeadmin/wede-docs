---
title: Parsers
description: Configure how event data is structured, encoded and transmitted across channels - including offline via structured protocols
---
...
In environments where connectivity is unreliable, every byte counts. When an event cannot be sent via REST, Wede falls back to structured protocols. A parser ensures that the most critical fields are encoded efficiently - fitting into as few SMS segments as possible - while preserving data integrity and legal compliance.
...
Each field is encoded as compact key-value pairs. The field code is a short identifier (up to 8 characters) that minimises transmission size.
Fields marked `offline_capable: false` are excluded when transmitting via structured protocols.
